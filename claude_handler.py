...
logger = get_logger("claude_handler")

# Adiciona o diretório do SDK ao path usando configuração
sdk_path = Path(settings.claude_sdk_path).resolve()
if sdk_path.exists():
    sys.path.insert(0, str(sdk_path))
    logger.info(f"Claude SDK path added: {sdk_path}")
else:
    logger.warning(f"Claude SDK path not found: {sdk_path}")

try:
    # Adiciona ambos os caminhos possíveis do SDK
    sdk_src_path = "/app/claude-code-sdk-python/src"
    sdk_root_path = "/app/claude-code-sdk-python"
    ...
    # Import do cliente principal - tenta vários caminhos
    try:
        import client
        ClaudeSDKClient = client.ClaudeSDKClient
    except ImportError:
        try:
    ...
    # Import dos tipos - tenta vários caminhos
    try:
        import sdk_types
        ...
        SystemMessage = sdk_types.SystemMessage
        ToolUseBlock = sdk_types.ToolUseBlock
        ToolResultBlock = sdk_types.ToolResultBlock
        ClaudeCodeOptions = sdk_types.ClaudeCodeOptions
    except ImportError:
        try:
            from src import sdk_types
            AssistantMessage = sdk_types.AssistantMessage
            TextBlock = sdk_types.TextBlock
            ResultMessage = sdk_types.ResultMessage
            UserMessage = sdk_types.UserMessage
    ...
    __version__ = "0.0.20"
    logger.info(f"Claude SDK loaded successfully, version: {__version__}")
...
class SessionConfig:
    ...
    permission_mode: str = field(default_factory=lambda: settings.claude_permission_mode)
    cwd: Optional[str] = field(default_factory=lambda: settings.claude_default_cwd)
...
class ClaudeHandler:
    ...
    def __init__(self):
        ...
        logger.info("ClaudeHandler initialized")
    ...
    async def send_message(
        ...
        logger.debug(f"Sending message to session {session_id}: {message[:50]}...")
